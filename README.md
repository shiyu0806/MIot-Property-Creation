...
小米 MIoT 平台**服务层 + 属性层 + 自定义自动化**一体化管理工具。
GUI 版本（`miot_gui.py`）将三套功能整合为统一界面，命令行版本保持原有用法不变。
...
### 🤖 自动化（`miot_automation_core.py`）
- 📤 **导出自动化** — 从平台拉取自定义自动化列表，生成 Excel
- 📥 **创建自动化** — 从 Excel 批量创建自定义自动化，自动检查标准场景
- 🧪 支持 dry-run 预检
